...
async function loadVideos() {
  try {
    ...
    if (data.items) {
      ...
      container.innerHTML = `<p class="text-red-500">Error: ${data.error?.message || "No se encontraron videos."}</p>`;
      console.error("Respuesta de la API:", data);
    }
  } catch (err) {
    console.error("Error en la petición:", err);
    document.getElementById("videos").innerHTML = `<p class="text-red-500">Error al cargar los videos.</p>`;
  }
}
loadVideos();
